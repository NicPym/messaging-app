  ...
  try {
    ...
    if (data.token) {
      ...
      socketManager.registerEvent("new message", (message) =>
        chatService.messageReceived(message)
      );
      setInnerHtml(
        "personTo",
        `<div class="d-flex flex-fill" style="width:0;"></div><a style="margin:0 30px;font-size:20px;text-decoration:none;color:black" href="http://localhost:8080/auth/logout" id="loginBtn">Logout</a></div>`
      );
      ...
      prependHtml(
        "personTo",
        `<label style="margin:0 10px;font-size:20px;">Login to see chats and messsages!</label>`
      );
    }
  } catch (err) {
    console.log(err);
